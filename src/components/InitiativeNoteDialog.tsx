...
interface InitiativeNoteDialogProps {
  initiative: Initiative;
  onClose: () => void;
}

export const InitiativeNoteDialog: React.FC<InitiativeNoteDialogProps> = ({ 
  initiative, 
  onClose 
}) => {
  const { addNote, updateNote, deleteNote, getNotesByInitiativeId } = useNoteStore();
  const notes = getNotesByInitiativeId(initiative.id);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingNote) {
      ...
      addNote(initiative.id, content, hasAction, 'initiative');
    }
